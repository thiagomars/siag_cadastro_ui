...
import { getSelectStatusAreaArmazenagem } from "../../../services/status.ts";
...
export default function SelectStatusAreaArmazenagem({ children }: { children: React.ReactNode }): JSX.Element {
    return (
        <>
            {children}
        </>
...
const Single = (props: Props): JSX.Element => {
    ...
    useEffect(() => {
        filtroOpcoesStatusAreaArmazenagem("");
    }, [])

    const filtroOpcoesStatusAreaArmazenagem = useDebounce((pesquisa: string) => carregaSelectStatusAreaArmazenagem(pesquisa), 500);

    async function carregaSelectStatusAreaArmazenagem(pesquisa: string): Promise<void> {
        const dados = {
            pesquisa: pesquisa
        };

        const response = await getSelectStatusAreaArmazenagem(dados);
    ...
    return (
        <Formulario.InputSelect
            name={name || "statusAreaArmazenagem"}
            label={label || "Status"}
            subTitulo={subTitulo || ""}
            control={control}
            disabled={disabled}
            opcional={opcional || false}
            onInputChange={filtroOpcoesStatusAreaArmazenagem}
...
const Multi = (props: Props): JSX.Element => {
    ...
    const filtroOpcoesStatusAreaArmazenagem = useDebounce((pesquisa: string) => carregaSelectDistribuidores(pesquisa), 500);
...
SelectStatusAreaArmazenagem.Single = Single;
SelectStatusAreaArmazenagem.Multi = Multi;